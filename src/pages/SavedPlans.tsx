...
export default function SavedPlans() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Saved MVP Plans</h1>
        <p className="text-sm text-muted-foreground mt-1">Your generated MVP plans, saved for reference</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-card p-12 text-center">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">Generate an MVP plan from any opportunity to get started.</p>
          <button onClick={() => navigate("/opportunities")} className="mt-4 h-9 px-4 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity">
        ...
        <div className="space-y-3">
          {plans.map((plan, i) => {
            ...
            const title = plan.opportunities?.title || "Untitled Opportunity";
            const niche = plan.opportunities?.niche || "Unknown";
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground mt-0.5">{niche} · {features.length} features</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => navigate(`/opportunities/${plan.opportunity_id}`)}
